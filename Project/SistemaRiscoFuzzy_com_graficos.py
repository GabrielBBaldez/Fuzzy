...
# Importar o módulo de geração de gráficos
from generate_fuzzy_plots import generate_membership_plots, generate_dynamic_plots
...
class SistemaRiscoFuzzy:
    def __init__(self):
        ...
        self.historico_avaliacoes = []  # Renomeado para evitar conflito com a variável fuzzy
        # Gerar gráficos estáticos das funções de pertinência
        self.plots = generate_membership_plots(self)
    ...
    def avaliar_cliente(self, dados):
        ...
        try:
            ...
            # Gerar gráficos dinâmicos com os valores atuais
            dynamic_plots = generate_dynamic_plots(self, dados)
        ...
        # Salvar no histórico
        self.historico_avaliacoes.append(resultado)
...
<head>
    ...
    <style>
        * {
        ...
        .form-card, .result-card, .about-card, .plots-card {
            background: white;
            padding: 30px;
            border-radius: 15px;
            box-shadow: 0 10px 30px rgba(0,0,0,0.1);
...
<body>
    <div class="container">
        ...
        <div class="about-card">
            <div class="tabs">
                ...
                <div class="tab" onclick="openTab('funcoes')">Funções de Pertinência</div>
            ...
            <div id="simulador" class="tab-content active">
                ...
                <div class="dynamic-plots" id="dynamicPlotsSection" style="display: none;">
                    <h3>Visualização das Funções de Pertinência com Valores Atuais</h3>
                    <div class="plots-container" id="dynamicPlotsContainer">
                        <!-- Gráficos dinâmicos serão inseridos aqui -->
                    </div>
                </div>
            ...
            <div id="funcoes" class="tab-content">
                <h2>Funções de Pertinência do Sistema Fuzzy</h2>
                <p>As funções de pertinência definem como os valores numéricos são mapeados para conjuntos fuzzy, permitindo o tratamento de conceitos vagos como "renda alta" ou "bom histórico". Abaixo estão os gráficos das funções de pertinência utilizadas neste sistema.</p>
                
                <div class="plot-tabs">
                    <div class="plot-tab active" onclick="openPlotTab('entrada')">Variáveis de Entrada</div>
                    <div class="plot-tab" onclick="openPlotTab('saida')">Variável de Saída</div>
                </div>
                
                <div id="entrada" class="plot-content active">
                    <div class="plots-container">
                        <div class="plot-item">
                            <h3>Renda Mensal</h3>
                            <img src="{{ plots.renda }}" alt="Funções de Pertinência - Renda" class="plot-img">
                        </div>
                        <div class="plot-item">
                            <h3>Score de Crédito</h3>
                            <img src="{{ plots.historico }}" alt="Funções de Pertinência - Histórico" class="plot-img">
                        </div>
                        <div class="plot-item">
                            <h3>Idade</h3>
                            <img src="{{ plots.idade }}" alt="Funções de Pertinência - Idade" class="plot-img">
                        </div>
                        <div class="plot-item">
                            <h3>Tempo de Emprego</h3>
                            <img src="{{ plots.tempo_emprego }}" alt="Funções de Pertinência - Tempo de Emprego" class="plot-img">
                        </div>
                        <div class="plot-item">
                            <h3>Percentual de Dívidas</h3>
                            <img src="{{ plots.dividas }}" alt="Funções de Pertinência - Dívidas" class="plot-img">
                        </div>
                    </div>
                </div>
                
                <div id="saida" class="plot-content">
                    <div class="plots-container">
                        <div class="plot-item" style="grid-column: span 2;">
                            <h3>Risco de Crédito (Saída)</h3>
                            <img src="{{ plots.risco }}" alt="Funções de Pertinência - Risco" class="plot-img">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        // Controle de abas para os gráficos
        function openPlotTab(tabName) {
            // Ocultar todos os conteúdos
            const tabContents = document.getElementsByClassName('plot-content');
            for (let i = 0; i < tabContents.length; i++) {
                tabContents[i].classList.remove('active');
            }
            
            // Remover classe ativa de todas as abas
            const tabs = document.getElementsByClassName('plot-tab');
            for (let i = 0; i < tabs.length; i++) {
                tabs[i].classList.remove('active');
            }
        ...
        document.getElementById('riskForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                mostrarGraficosDinamicos(resultado);
                carregarHistorico();
        ...
        function mostrarGraficosDinamicos(resultado) {
            if (!resultado.plots || Object.keys(resultado.plots).length === 0) {
                return;
            }
            
            const dynamicPlotsSection = document.getElementById('dynamicPlotsSection');
            const dynamicPlotsContainer = document.getElementById('dynamicPlotsContainer');
            
            let html = '';
            
            // Adicionar cada gráfico dinâmico
            for (const [key, plotUrl] of Object.entries(resultado.plots)) {
                let title = '';
                ...
                html += `
                    <div class="plot-item">
                        <h3>${title}</h3>
                        <img src="${plotUrl}" alt="Função de Pertinência - ${title}" class="plot-img">
                    </div>
                `;
            }
            
            dynamicPlotsContainer.innerHTML = html;
            dynamicPlotsSection.style.display = 'block';
        }
...
def index():
    """Página principal"""
    return render_template_string(HTML_TEMPLATE, plots=sistema.plots)
...
def historico():
    ...
    return jsonify(sistema.historico_avaliacoes)

@app.route('/limpar', methods=['POST'])
def limpar_historico():
    """Endpoint para limpar histórico"""
    sistema.historico_avaliacoes.clear()
